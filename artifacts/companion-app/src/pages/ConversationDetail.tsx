...
export default function ConversationDetail({ companionId }: ConversationDetailProps) {
  ...
  // Group messages — insert timestamp dividers every ~10 messages
  const messages = conversation?.messages ?? [];
  const shouldShowTimestamp = (i: number) =>
    i === 0 ||
    new Date(messages[i].createdAt).getTime() - new Date(messages[i - 1].createdAt).getTime() > 10 * 60 * 1000;
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-background max-w-md mx-auto relative overflow-hidden">
      ...
      >
        ...
        {isLoading ? (
          ...
          <>
            <div className="relative">
              <img
                src={conversation?.companion.avatarUrl}
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                <div className="h-1 w-16 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
                  <div
                    ...
                    style={{
                      width: `${Math.min(conversation?.affinity ?? 0, 100)}%`,
                      background: "linear-gradient(90deg, hsl(348 76% 40%), hsl(351 88% 62%))",
                    }}
                  />
                </div>
                <span className="text-[10px] font-medium" style={{ color: "hsl(348 76% 65%)" }}>
                  {t("conversations.affinity_label")} {conversation?.affinity ?? 0}
                </span>
              </div>
            </div>
          </>
      ...
      {/* Messages */}
      <main className="flex-1 overflow-y-auto px-4 py-4 pb-28 space-y-2">
        {isLoading ? (
          ...
          <AnimatePresence initial={false}>
            {messages.map((msg, i) => (
              <div key={msg.id}>
                ...
                <ChatBubble
                  message={msg}
                  avatarUrl={conversation?.companion.avatarUrl ?? ""}
                />
              </div>
            ))}
          </AnimatePresence>
      ...
      {/* Continue in Bot Footer */}
      <div
        className="fixed bottom-0 left-0 right-0 max-w-md mx-auto px-4 py-4"
        style={{
          background: "linear-gradient(to top, rgba(8,2,4,0.98) 60%, transparent)",
        }}
      >
        <Button
        ...
        <p className="text-[10px] text-muted-foreground/50 text-center mt-2">
          {messages.length > 0 && `${messages.length} messages · showing last 50`}
        </p>
      </div>
    </div>
  );
